    ...
    >
      ...
      <td className="px-6 py-3">
        <div className="flex items-center gap-2">
          {role === "admin" && (
            <>
              <FromModal table="exam" type="update" data={item} />
              <FromModal table="exam" type="delete" id={item?.id} />
            </>
          )}
        </div>
      </td>
  ...
  return (
    <section className="bg-white flex-1 p-4 m-4 mt-0 rounded-md">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold hidden md:block">Exams</h2>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end ">
            ...
            {role === "admin" && <FromModal table="exam" type="create" />}
          </div>
        </div>
      </div>
      {/* list  */}
      <div>
        <Table columns={columns} randerRow={renderRow} data={examsData} />
      </div>
      {/* pagination  */}
      <div>
        <Pagination />
