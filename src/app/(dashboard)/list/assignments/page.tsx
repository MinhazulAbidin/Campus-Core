...
const AssignmentsListPage = () => {
  const renderRow = (item: Assignments) => (
    ...
    >
      ...
      <td className="hidden md:table-cell px-6 py-3">{item?.dueDate}</td>
      <td className="px-6 py-3">
        <div className="flex items-center gap-2">
          {role === "admin" && (
            <>
            <FromModal table="assignment" type="update" data={item?.id} />
            <FromModal table="assignment" type="delete" id={item?.id} />
            </>
          )}
        </div>
      </td>
  ...
  return (
    <section className="bg-white flex-1 p-4 m-4 mt-0 rounded-md">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold hidden md:block">Assignments</h2>
        <div className="flex flex-col md:flex-row items-center gap-4 w-full md:w-auto">
          ...
          <div className="flex items-center gap-4 self-end ">
            ...
            {role === "admin" && <FromModal table="assignment" type="create" />}
          </div>
        </div>
      </div>
      {/* list  */}
      <div>
        <Table columns={columns} randerRow={renderRow} data={assignmentsData} />
      </div>
      {/* pagination  */}
      <div>
        <Pagination />
